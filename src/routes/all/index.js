...
export default class All extends Component {
  ...
  addToFav(data) {
    ...
    localForage.getItem("myList").then(myList => {
      if (!myList) {
        const obj = {};
        obj[data.stateCode] = data;
        localForage.setItem("myList", obj);
        isAddedToMyFav.push(data.stateCode);
        this.setState({isAddedToMyFav}, () => {
          toast(`${data.state} is added to your list`);
        });
      }
      else if (myList[data.stateCode]) {
        delete myList[data.stateCode];
        localForage.setItem("myList", myList);
        isAddedToMyFav.splice(isAddedToMyFav.indexOf(data.stateCode), 1);
        this.setState({isAddedToMyFav}, () => {
          toast(`${data.state} is removed from your list`);
        });
      }
      else {
        myList[data.stateCode] = data;
        localForage.setItem("myList", myList);
        isAddedToMyFav.push(data.stateCode);
        this.setState({isAddedToMyFav}, () => {
          toast(`${data.state} is added to your list`);
  ...
  htmlSwitchInput(data) {
    ...
    return (
      <div class="add-to-card">
        {
          (isAddedToMyFav.indexOf(data.stateCode) > -1) ? (<div data-state={data.stateCode} onClick={this.addToFav.bind(this, data)}><svg fill="#4A4A4A" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
            <path d="M0 0h24v24H0z" fill="none"/>
            <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
          </svg></div>) : (<div data-state={data.stateCode} onClick={this.addToFav.bind(this, data)}><svg fill="#000000" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg">
            <path d="M0 0h24v24H0z" fill="none"/>
  ...
  htmlCards(result) {
    return (
      <div class="cards">
        <h3>{result.townname.toLowerCase()}</h3>
        <div><b>Petrol</b>: ₹ <span>{result.ms}</span></div>
        <div><b>Diesel</b>: ₹ <span>{result.hsd}</span></div>
      </div>
    );
  }
  ...
  htmlCardsContainer(data) {
    return data.map(state => {
      state.result = sort(state.result, "townname");
      return (
