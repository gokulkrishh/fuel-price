...
export default class MyList extends Component {
      ...
      .then(data => {
        if (!isEmpty(data)) {
          ...
          if (country_name === "India" && region_code !== "") {
            this.fetchCurrentStateFuel(region_code);
					}
					else {
						this.setState({
							myList: {},
  ...
  fetchCurrentStateFuel(regionCode, fromPush) {
    ...
    fetch(`${config.apiUrl}/fuelprice/${regionCode.toLowerCase()}`, options)
      .then(response => response.json())
      .then(response => {
        const [newResponse] = response;
        if (newResponse && !isEmpty(newResponse)) {
          if (!myList[newResponse.stateCode]) {
            myList[newResponse.stateCode] = newResponse;
  ...
  htmlCards(data, stateCode) {
    return (
      <div class="cards">
        <h3>{data.townname.toLowerCase()} {this.htmlRemoveIcon(data.townname, stateCode)}</h3>
        <div><b>Petrol</b>: ₹ <span>{data.ms}</span></div>
        <div><b>Diesel</b>: ₹ <span>{data.hsd}</span></div>
      </div>
    );
  }
  ...
  htmlPushSubIcon(data) {
    return (
      <span class="add-to-card push">
        {
          data.pushNotification ? (<svg fill="#4A4A4A" height="24" viewBox="0 0 24 24" width="24" onClick={this.togglePushSubscription.bind(this, "Unsubscribe", data.stateCode)}>
            <path d="M12 22c1.1 0 2-.9 2-2h-4c0 1.1.89 2 2 2zm6-6v-5c0-3.07-1.64-5.64-4.5-6.32V4c0-.83-.67-1.5-1.5-1.5s-1.5.67-1.5 1.5v.68C7.63 5.36 6 7.92 6 11v5l-2 2v1h16v-1l-2-2z"></path>
          </svg>) : (<svg title="Click to subscribe push notification" fill="#ccc" height="24" viewBox="0 0 24 24" width="24" onClick={this.togglePushSubscription.bind(this, "Subscribe", data.stateCode)}>
  ...
  htmlCardContainer(data) {
    ...
    if (!data) return;
    return Object.keys(data).sort().map(state => {
      data[state].result = sort(data[state].result, "townname");
      return (
